...
export class ServiceGroupsService {
  ...
  list(): Observable<ServiceGroup[]> {
    return this.makeRequest('GET', `/users/api/service-groups`);
  }

  createGroup(group: ServiceGroup): Observable<ServiceGroup> {
    return this.makeRequest('POST', `/users/api/service-groups`, group);
  }

  updateGroup(group: ServiceGroup): Observable<ServiceGroup> {
    return this.makeRequest('PUT', `/users/api/service-groups/${group._id}`, group);
  }

  removeGroup(groupId: string): Observable<any> {
    return this.makeRequest('DELETE', `/users/api/service-groups/${groupId}`);
  }

  listServiseConfigs(): Observable<any> {
    return this.makeRequest('GET', `/users/api/service-setups`);
  }
